...
class Scope {
	...
	String text;
	Rectangle rect;
	boolean showI, showV, showMax, showMin, showFreq, lockScale, plot2d,
			plotXY;
	...
	void draw2d(Graphics g) {
		...
		for (i = 0; i != rect.width; i++)
			pixels[i + rect.width * (rect.height / 2)] = 0xFF20AF41;
		int ycol = (plotXY) ? 0xFF20AF41 : 0xFFFFFF00;
	...
	void draw(Graphics g) {
		...
		if (pixels == null)
			return;
		int i;
		int col = (sim.printableCheckItem.getState()) ? 0xFFF2FFE3 : 0;
		for (i = 0; i != pixels.length; i++)
			pixels[i] = col;
		int x = 0;
		...
		double realMinV = 1e8;
		double realMinI = 1e8;
		int curColor = (sim.printableCheckItem.getState() ? 0xFFFFB12A :0xFFFFFF00);
		int voltColor = (value > 0) ? 0xFFFFFFFF : 0xFF20BF31;
		//hightlight curve if corressponding element is selected
		...
		for (ll = -100; ll <= 100; ll++) {
			...
			if (yl < 0 || yl >= rect.height - 1)
				continue;
			col = ll == 0 ? (sim.printableCheckItem.getState()?0xFFAABBAA:0xFF808080) : (sim.printableCheckItem.getState()?0xFFDDEEDD:0xFF353535);
			if (ll % 10 != 0) {
				col = (sim.printableCheckItem.getState()?0xFFD0E0D0:0xFF101010);
				if (!sublines)
					continue;
			}
			for (i = 0; i != rect.width; i++)
		...
		for (ll = 0;; ll++) {
			...
			if (tl < 0)
				continue;
			col = (sim.printableCheckItem.getState()?0xFFBBCCBB:0xFF1A1A1A);
			first = 0;
			if (((tl + gridStep / 4) % (gridStep * 10)) < gridStep) {
				col =  (sim.printableCheckItem.getState()?0xFF809080:0xFF808080);
				if (((tl + gridStep / 4) % (gridStep * 100)) < gridStep)
					col =  (sim.printableCheckItem.getState()?0xFFDDEEDD:0xFF3535C5);
			}
		...
		g.drawImage(image, rect.x, rect.y, null);
		 ...
		 if (sim.printableCheckItem.getState()){
			 g.setColor(new Color(56,137,68));
			 g.drawRect(rect.x, rect.y-1,rect.width, rect.height-1);
		 }
		...
		if (showMax) {
			if (value != 0)
				g.drawString(
						elm.getUnitText(realMaxV, elm.getScopeUnits(value)), x,
						yt);
			else if (showV)
		...
		if (showMin) {
			...
			if (value != 0)
				g.drawString(
						elm.getUnitText(realMinV, elm.getScopeUnits(value)), x,
						ym);
			else if (showV)
	...
	PopupMenu getMenu() {
		...
		if (elm instanceof TransistorElm) {
			...
			sim.scopeVceIcMenuItem.setState(value == VAL_VCE
					&& ivalue == VAL_IC);
	...
	void allocImage() {
		...
		if (sim.useBufferedImage) {
			try {
				...
				image = (Image) cstr
						.newInstance(new Object[] { new Integer(w),
								new Integer(h),
