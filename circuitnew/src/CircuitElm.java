...
public abstract class CircuitElm implements Editable {
	...
	static Color currentColor, voltageColor;
	...
	/**
     * flag if capacitor can be varied manually
     */
	boolean variable=false;
	/**
	 * values if element is set to variable (manually)
	 */
	...
	/**
	 * create bounding box of element
	 */
	...
	/**
	 * returns string tokens of the object to export its info to file
	 * @return
	 */
	String dump() {
		int t = getDumpType();
	...
	boolean allowMove(int dx, int dy) {
		...
		for (i = 0; i != sim.elmList.size(); i++) {
			...
			if (this instanceof VoltmeterElm && !(ce instanceof VoltmeterElm)) return true;
			if (!(this instanceof VoltmeterElm) && (ce instanceof VoltmeterElm)) return true;
	...
	void setInterpolatedValue(double v){
		if (v>1)v=1;
		if (v<0)v=0;
		v=(minValue*(1-v) +maxValue*v);
		setValue(v);
	}
