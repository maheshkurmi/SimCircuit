...
public class CirSim extends JFrame implements ComponentListener,
		...
		ItemListener, KeyListener, MouseWheelListener {
	...
	MenuItem newItem, exportItem, exportLinkItem, importItem, exitItem,
			undoItem, undoItemPopUp, redoItem, redoItemPopUp, cutItem,
			copyItem, pasteItem, pasteItemPopUp, selectAllItem, optionsItem;
	Menu optionsMenu;
	...
	CheckboxMenuItem drawGridCheckItem;
	...
	boolean isMac;
	/** flag to disable dragging of element at scope */
	boolean avoidDrag = false;
	String ctrlMetaKey;
	double t;
	int pause = 10;
	int scopeSelected = -1;
	...
	// controls
	JPanel pnlToolBar;
	private JToolBar barElements;
	private JToolBar barEditMenu;
	...
	/** The simulation speed up button */
	private JButton btnSpeedUp;

	/** The simulation speed Down button */
	...
	private JToggleButton[] tglAddElements;

	private KnobElm knob;
	private boolean moveKnob = false;
	/** Status bar */
	...
	CirSim(Circuit a) {
		super("SymCiruit v1.0.0");
		applet = a;
		useFrame = false;
		knob = new KnobElm(0, this);
	...
	public void init() {
		...
		boolean printable = false; // hausen: changed from false to true
		...
		try {
			...
			// completely ignore the error and just use the default look and
			// feel
		}
		// setup the layout
		pnlMain = new JPanel();
		pnlMain.setPreferredSize(new Dimension(900, 600));
		pnlMain.setLayout(new CircuitLayout());
		...
		cv.setPreferredSize(new Dimension(900, 800));
		// main.add(cv);
		...
		main.add(cv, BorderLayout.CENTER);
		main.add(this.pnlToolBar, BorderLayout.NORTH);
		statusBar = new StatusBarPanel();
		this.add(statusBar, BorderLayout.SOUTH);
		// Chess chessPanel = new Chess();
		// add(chessPanel, BorderLayout.CENTER);

		// main.add(pnlMain,BorderLayout.SOUTH);
		...
		m.add(importItem = getMenuItem("Open .."));
		m.addSeparator();
		m.add(exportItem = getMenuItem("Save"));
		m.add(exportLinkItem = getMenuItem("Save As .."));
		m.addSeparator();
		...
		// create popup undo redo menus
		undoItemPopUp = getMenuItem("Undo");
		undoItemPopUp.setActionCommand("undo");
		redoItemPopUp = getMenuItem("Redo");
		redoItemPopUp.setActionCommand("redo");
		...
		m.addSeparator();
		pasteItemPopUp = getMenuItem("Paste");
		pasteItemPopUp.setShortcut(new MenuShortcut(KeyEvent.VK_V));
		pasteItemPopUp.setEnabled(false);
		...
		m.add(drawGridCheckItem = getCheckItem("Draw Grid"));
		m.addSeparator();
		...
		inputMenu.add(getClassCheckItem("Add Square Wave (1-terminal)",
				"SquareRailElm"));
		inputMenu.add(getClassCheckItem("Add Voltmeter", "VoltmeterElm"));
		inputMenu.add(getClassCheckItem("Add Ammeter", "AmmeterElm"));
		inputMenu.add(getClassCheckItem("Add Galvanometer", "GalvanometerElm"));
		...
		inputMenu.add(getClassCheckItem("Add Bulb", "BulbElm"));
		...
		gateMenu.addSeparator();
		gateMenu.add(getClassCheckItem("Add Logic Input", "LogicInputElm"));
		gateMenu.add(getClassCheckItem("Add Logic Output", "LogicOutputElm"));
		...
		createMainPopUpMenu(mainMenu);
		mainMenu.addSeparator();
		mainMenu.add(getCheckItem("Select/Drag Selected (space)", "Select"));
		main.add(mainMenu);

		pnlMain.add(resetButton = new Button("Reset"));
		...
		pnlMain.add(stoppedCheck);

		pnlMain.add(new Label("Simulation Speed", Label.CENTER));

		// was max of 140
		pnlMain.add(speedBar = new Scrollbar(Scrollbar.HORIZONTAL, 3, 1, 0, 260));
		speedBar.addAdjustmentListener(this);

		pnlMain.add(new Label("Current Speed", Label.CENTER));
		currentBar = new Scrollbar(Scrollbar.HORIZONTAL, 50, 1, 1, 100);
		currentBar.addAdjustmentListener(this);
		pnlMain.add(currentBar);

		pnlMain.add(powerLabel = new Label("Power Brightness", Label.CENTER));
		pnlMain.add(powerBar = new Scrollbar(Scrollbar.HORIZONTAL, 50, 1, 1,
				100));
		powerBar.addAdjustmentListener(this);
		powerBar.disable();
		powerLabel.disable();

		pnlMain.add(new Label(" "));
		...
		if (useFrame) {
			...
			if (!powerCheckItem.getState()) {
				pnlMain.remove(powerBar);
				pnlMain.remove(powerLabel);
				pnlMain.validate();
			}
			hide();
			handleResize();
	...
	private void createToolBar() {
		...
		barEditMenu = new JToolBar("toolbar.simulation", JToolBar.HORIZONTAL);
		barEditMenu.setFloatable(false);
		...
		this.btnOpen.setToolTipText("Import File");
		...
		this.btnExport.setActionCommand("exporttopng");
		this.btnExport.setToolTipText("Export as png");
		...
		barEditMenu.add(this.btnNew);
		barEditMenu.add(this.btnOpen);
		barEditMenu.add(this.btnSave);
		barEditMenu.addSeparator();

		barEditMenu.add(this.btnExport);
		barEditMenu.add(this.btnPrint);
		barEditMenu.addSeparator();
		...
		this.btnSpeedUp = new JButton(Icons.SPEED_UP);
		this.btnSpeedUp.setToolTipText("Increase Simulation Speed");
		this.btnSpeedUp.setActionCommand("speed-up");
		this.btnSpeedUp.addActionListener(this);

		this.btnSpeedDown = new JButton(Icons.SPEED_DOWN);
		this.btnSpeedDown.setToolTipText("Decrease Simulation Speed");
		this.btnSpeedDown.setActionCommand("speed-down");
		this.btnSpeedDown.addActionListener(this);
		...
		this.btnUnDo.setToolTipText("Undo Action");
		this.btnUnDo.setActionCommand("undo");
		this.btnUnDo.addActionListener(this);

		this.btnReDo = new JButton(Icons.REDO);
		this.btnReDo.setToolTipText("Redo Action");
		this.btnReDo.setActionCommand("redo");
		this.btnReDo.addActionListener(this);

		this.btnDelete = new JButton(Icons.DELETE);
		this.btnDelete.setActionCommand("delete");
		this.btnDelete.setToolTipText("Delete Selected Element(s)");
		this.btnDelete.addActionListener(this);

		this.btnCut = new JButton(Icons.CUT);
		this.btnCut.setActionCommand("cut");
		this.btnCut.setToolTipText("Cut Selected Element(s)");
		this.btnCut.addActionListener(this);

		this.btnCopy = new JButton(Icons.COPY);
		this.btnCopy.setActionCommand("copy");
		this.btnCopy.setToolTipText("Copy Selected Elements(s)");
		this.btnCopy.addActionListener(this);

		this.btnPaste = new JButton(Icons.PASTE);
		this.btnPaste.setActionCommand("paste");
		this.btnPaste.setToolTipText("Paste");
		this.btnPaste.addActionListener(this);

		barEditMenu.add(this.btnZoomOut);
		barEditMenu.add(this.btnZoomIn);
		barEditMenu.addSeparator();
		barEditMenu.add(this.btnSpeedDown);
		barEditMenu.add(this.btnSpeedUp);
		barEditMenu.addSeparator();
		
		barEditMenu.add(this.btnUnDo);
		barEditMenu.add(this.btnReDo);
		barEditMenu.addSeparator();
		barEditMenu.add(this.btnCut);
		barEditMenu.add(this.btnCopy);
		barEditMenu.add(this.btnPaste);
		barEditMenu.addSeparator();
		barEditMenu.add(this.btnDelete);

		// create the preferences toolbar
		this.tglAntiAliasing = new JToggleButton(Icons.ANTIALIAS);
		this.tglAntiAliasing.setToolTipText("Toggles anti-aliasing");
		...
		barElements = new JToolBar("toolbar.preferences",
				JToolBar.HORIZONTAL);
		barElements.setFloatable(false);
		...
		this.btnReset.setToolTipText("Reset simulation");
		...
		barElements.add(this.btnStart);
		barElements.add(this.btnStep);
		barElements.add(this.btnStop);
		barElements.add(this.btnReset);
		barElements.addSeparator();

		barElements.add(this.tglMoveBody);
		barElements.addSeparator();

		this.tglAddElements = new JToggleButton[24];
		for (int i = 0; i < 24; i++) {
			this.tglAddElements[i] = new JToggleButton();
			this.tglAddElements[i].setActionCommand("addelm" + i);
			this.tglAddElements[i].addActionListener(this);
			switch (i) {
			case 0:
				this.tglAddElements[i].setIcon(Icons.WIRE);
				this.tglAddElements[i].setToolTipText("Wire");
				break;
			case 1:
				this.tglAddElements[i].setIcon(Icons.RESISTOR);
				this.tglAddElements[i].setToolTipText("Resistor");
				break;
			case 2:
				this.tglAddElements[i].setIcon(Icons.CAPACITOR);
				this.tglAddElements[i].setToolTipText("Capacitor");
				break;
			case 3:
				this.tglAddElements[i].setIcon(Icons.INDUCTOR);
				this.tglAddElements[i].setToolTipText("Inductor");
				break;
			case 4:
				this.tglAddElements[i].setIcon(Icons.BATTERY);
				this.tglAddElements[i].setToolTipText("Battery");
				break;
			case 5:
				this.tglAddElements[i].setIcon(Icons.CURRENTSOURCE);
				this.tglAddElements[i].setToolTipText("Current Source");
				break;
			case 6:
				this.tglAddElements[i].setIcon(Icons.ACSOURCE);
				this.tglAddElements[i].setToolTipText("AC Source");
				break;
			case 7:
				this.tglAddElements[i].setIcon(Icons.VIN);
				this.tglAddElements[i]
						.setToolTipText("Single terminal Voltage Source");
				break;
			case 8:
				this.tglAddElements[i].setIcon(Icons.GROUND);
				this.tglAddElements[i].setToolTipText("Ground");
				break;
			case 9:
				this.tglAddElements[i].setIcon(Icons.RHOESTAT);
				this.tglAddElements[i].setToolTipText("Potentiometer");
				break;
			case 10:
				this.tglAddElements[i].setIcon(Icons.SWITCH1);
				this.tglAddElements[i].setToolTipText("2 Terminal Switch");
				break;
			case 11:
				this.tglAddElements[i].setIcon(Icons.SWITCH2);
				this.tglAddElements[i].setToolTipText("3 Terminal Switch");
				break;
			case 12:
				this.tglAddElements[i].setIcon(Icons.AMMETER);
				this.tglAddElements[i].setToolTipText("Ammeter");
				break;
			case 13:
				this.tglAddElements[i].setIcon(Icons.VOLTMETER);
				this.tglAddElements[i].setToolTipText("Voltmeter");
				break;
			case 14:
				this.tglAddElements[i].setIcon(Icons.VOUT);
				this.tglAddElements[i]
						.setToolTipText("Single Point Voltage OutPut");
				break;
			case 15:
				this.tglAddElements[i].setIcon(Icons.GALVANOMETER);
				this.tglAddElements[i].setToolTipText("Galvanometer");
				break;
			case 16:
				this.tglAddElements[i].setIcon(Icons.TRANSFORMER);
				this.tglAddElements[i].setToolTipText("Transformer");
				break;
			case 17:
				this.tglAddElements[i].setIcon(Icons.BULB);
				this.tglAddElements[i].setToolTipText("Bulb");
				break;
			case 18:
				this.tglAddElements[i].setIcon(Icons.AND_GATE);
				break;
			case 19:
				this.tglAddElements[i].setIcon(Icons.OR_GATE);
				break;
			case 20:
				this.tglAddElements[i].setIcon(Icons.NOR_GATE);
				break;
			case 21:
				this.tglAddElements[i].setIcon(Icons.NAND_GATE);
					break;
			case 22:
				this.tglAddElements[i].setIcon(Icons.XOR_GATE);
				break;
			case 23:
				this.tglAddElements[i].setIcon(Icons.DIODE);
				this.tglAddElements[i].setToolTipText("Diode");
				break;
			default:
				this.tglAddElements[i].setIcon(Icons.WIRE);
				break;
		...
		for (int i = 0; i < 18; i++) {
			barElements.add(this.tglAddElements[i]);
			if (i == 3 || i == 6 || i == 9 || i == 11 || i == 15)
				barElements.addSeparator();
		}

		pnlToolBar = new JPanel();
		pnlToolBar.setLayout(new GridLayout(2, 1));
		pnlToolBar.add(barEditMenu);
		pnlToolBar.add(barElements);

		elmPopUp = new JPopupMenu();
		jMnuItems = new JMenuItem[5];
		jMnuItems[0] = new JMenuItem("Edit");
		jMnuItems[0].setIcon(Icons.EDIT);
		jMnuItems[0].setActionCommand("edit");
		jMnuItems[0].addActionListener(this);
		elmPopUp.add(jMnuItems[0]);
		elmPopUp.addSeparator();
		jMnuItems[1] = new JMenuItem("View in Scope");
		jMnuItems[1].setActionCommand("scope");
		jMnuItems[1].setIcon(Icons.SCOPE);
		jMnuItems[1].addActionListener(this);
		elmPopUp.add(jMnuItems[1]);
		elmPopUp.addSeparator();
		jMnuItems[2] = new JMenuItem("Cut");
		jMnuItems[2].setActionCommand("cut");
		jMnuItems[2].setIcon(Icons.CUT);
		jMnuItems[2].addActionListener(this);
		elmPopUp.add(jMnuItems[2]);
		jMnuItems[3] = new JMenuItem("Copy");
		jMnuItems[3].setIcon(Icons.COPY);
		jMnuItems[3].setActionCommand("copy");
		jMnuItems[3].addActionListener(this);
		elmPopUp.add(jMnuItems[3]);
		elmPopUp.addSeparator();
		jMnuItems[4] = new JMenuItem("Delete");
		jMnuItems[4].setIcon(Icons.DELETE);
		jMnuItems[4].setActionCommand("delete");
		jMnuItems[4].addActionListener(this);
		elmPopUp.add(jMnuItems[4]);
	}

	void createMainPopUpMenu(PopupMenu mainMenu) {
		Menu addElementsMenu = new Menu("Add Basic Elements");
		Menu addInputsMenu = new Menu("Add Input");
		Menu addOutputsMenu = new Menu("Add Output");
		Menu addSwitchMenu = new Menu("Add Switch");
		Menu addMiscMenu = new Menu("Add Miscleneous");
		Menu editMenu = new Menu("Edit");
		Menu dragModeMenu = new Menu("DragMode");
		mainMenu.add(addElementsMenu);
		mainMenu.addSeparator();
		mainMenu.add(addInputsMenu);
		mainMenu.add(addOutputsMenu);
		mainMenu.addSeparator();
		mainMenu.add(addSwitchMenu);
		mainMenu.add(addMiscMenu);
		mainMenu.addSeparator();
		mainMenu.add(editMenu);
		mainMenu.addSeparator();
		mainMenu.add(dragModeMenu);
		addElementsMenu.add(getClassCheckItem("Add Wire", "WireElm"));
		addElementsMenu.addSeparator();
		addElementsMenu.add(getClassCheckItem("Add Resistor", "ResistorElm"));
		addElementsMenu.add(getClassCheckItem("Add Capacitor", "CapacitorElm"));
		addElementsMenu.add(getClassCheckItem("Add Inductor", "InductorElm"));

		addInputsMenu.add(getClassCheckItem("Add Voltage Source (2-terminal)",
				"DCVoltageElm"));
		addInputsMenu.add(getClassCheckItem("Add Current Source (2-terminal)",
				"CurrentElm"));
		addInputsMenu.add(getClassCheckItem("Add A/C Source (2-terminal)",
				"ACVoltageElm"));
		addInputsMenu.addSeparator();
		addInputsMenu.add(getClassCheckItem("Add Voltage Source (1-terminal)",
				"RailElm"));
		addInputsMenu.add(getClassCheckItem("Add AC Voltage Source (1-terminal)",
				"ACRailElm"));

		addInputsMenu.addSeparator();
		addInputsMenu.add(getClassCheckItem("Add Ground", "GroundElm"));

		addOutputsMenu.add(getClassCheckItem("Add Ammeter", "AmmeterElm"));
		addOutputsMenu.add(getClassCheckItem("Add Voltmeter", "VoltmeterElm"));
		addOutputsMenu.add(getClassCheckItem("Add Galvanometer",
				"GalvanometerElm"));

		addOutputsMenu.addSeparator();
		addOutputsMenu.add(getClassCheckItem("Add Analog Output", "OutputElm"));

		addSwitchMenu.add(getClassCheckItem("Add Switch", "SwitchElm"));
		addSwitchMenu.addSeparator();
		addSwitchMenu.add(getClassCheckItem("Add SPDT Switch", "Switch2Elm"));

		editMenu.add(pasteItemPopUp);
		editMenu.addSeparator();
		editMenu.add(undoItemPopUp);
		editMenu.add(redoItemPopUp);
		editMenu.addSeparator();
		editMenu.add(selectAllItem = getMenuItem("Select All"));
		selectAllItem.setShortcut(new MenuShortcut(KeyEvent.VK_A));

		addMiscMenu.add(getClassCheckItem("Add Diode", "DiodeElm"));
		addMiscMenu.addSeparator();
		addMiscMenu.add(getClassCheckItem("Add Potentiometer", "PotElm"));
		addMiscMenu.add(getClassCheckItem("Add Transformer", "TransformerElm"));
		addMiscMenu.add(getClassCheckItem("Add Bulb ", "BulbElm"));
		addMiscMenu.addSeparator();
		addMiscMenu.add(getClassCheckItem("Add Text", "TextElm"));

		dragModeMenu.add(getCheckItem("Drag Selected", "DragSelected"));
		dragModeMenu.addSeparator();
		dragModeMenu.add(getCheckItem("Drag All (Alt-drag)", "DragAll"));
		dragModeMenu.add(getCheckItem(isMac ? "Drag Row (Alt-S-drag, S-right)"
				: "Drag Row (S-right)", "DragRow"));
		dragModeMenu.add(getCheckItem(
				isMac ? "Drag Column (Alt-\u2318-drag, \u2318-right)"
						: "Drag Column (C-right)", "DragColumn"));
		dragModeMenu.addSeparator();
		dragModeMenu.add(getCheckItem("Resize Element (" + ctrlMetaKey
				+ "-drag)", "DragPost"));
	...
	JMenuItem getJMenuItem(String s, String ac, ImageIcon icon) {
	...
	void handleResize() {
		...
		if (winSize.width == 0)
			return;
		dbimage = cv.createImage(winSize.width, winSize.height);
		int h = winSize.height / 5;
		/*
		 * if (h < 128 && winSize.height > 300) h = 128;
		 */
		// if (camera.scale<1){
		circuitArea = new Rectangle(0, 0, (int) (winSize.width / camera.scale),
				(int) ((winSize.height - h) / camera.scale));
		// }else
		...
		int dx = gridMask
				& (((int) (circuitArea.width * camera.scale) - (maxx - minx)) / 2 - minx);
		int dy = gridMask
				& (((int) (circuitArea.height * camera.scale) - (maxy - miny)) / 2 - miny);
		if (dx + minx < 0)
			dx = gridMask & (-minx);
		if (dy + miny < 0)
			dy = gridMask & (-miny);
	...
	private void drawGrid(Graphics g, int gridSize, int width, int height) {
	...
	public void updateCircuit(Graphics realg) {
		...
		if (printableCheckItem.getState()) {
			...
			//CircuitElm.currentColor=Color.orange;
			//g.setColor(Color.white);
			CircuitElm.whiteColor = Color.GREEN.darker().darker();
			CircuitElm.lightGrayColor = Color.gray;
			CircuitElm.currentColor=Color.orange;
			g.setColor(Color.white);
		} else {
			CircuitElm.selectColor=Color.YELLOW;
			CircuitElm.whiteColor = Color.white;
			CircuitElm.lightGrayColor = Color.white;//Color.lightGray;
			CircuitElm.currentColor=Color.yellow;
			g.setColor(new Color(0,64,0));//Color.black);
		}
		g.fillRect(0, 0, winSize.width, winSize.height);

		// transform graphics by camera
		g.setTransform(camera.getTransform());

		if (drawGridCheckItem.getState())
			drawGrid(g, gridSize, (int) (winSize.width / camera.scale),
					(int) (winSize.height * 4 / 5 / camera.scale));
/*
		if (printableCheckItem.getState()) {
			int h1 = (int) (winSize.height - circuitArea.height * camera.scale + 5);
			g.setColor(new Color(250, 255, 240));
			g.fillRect(0, winSize.height - h1, winSize.width, h1);
			g.setColor(new Color(40, 185, 80));
			g.drawRect(0, winSize.height - h1, winSize.width, h1);
		}
*/
		...
		// reset transform do not let scopes zoom
		...
		g.fillRect(0, winSize.height * 4 / 5, winSize.width, winSize.height / 5);
		if (printableCheckItem.getState()) {
			int h1 = (int) (winSize.height - circuitArea.height * camera.scale + 5);
			g.setColor(new Color(250, 255, 240));
			g.fillRect(0, winSize.height - h1, winSize.width, h1);
		...
		if (stopMessage != null) {
			g.drawString(stopMessage, 10,
					(int) (circuitArea.height * camera.scale));
		} else {
			if (circuitBottom == 0)
				calcCircuitBottom();
			...
			ybase = min(ybase, (int) (circuitArea.height * camera.scale));
			ybase = max(ybase, circuitBottom);
			for (i = 0; info[i] != null; i++)
				g.drawString(info[i], x, ybase + 15 * (i + 1));

			if (knob.element != null) {
				int h = (int) (winSize.height - circuitArea.height
						* camera.scale - 5);
				knob.element.getInfo(info);
				g.drawString(info[0], winSize.width - h, ybase + 15);
				knob.setRect(new Rectangle(winSize.width - h, ybase + 23,
						h - 25, h - 25));
			}
		...
		if (selectedArea != null) {
			...
			g.setColor(new Color(21, 183, 255, 55));
			g.fillRect(selectedArea.x, selectedArea.y, selectedArea.width,
					selectedArea.height);
			g.setColor(new Color(21, 183, 255, 155));// Color.blue.brighter());
	...
	void setupScopes() {
		...
		while (scopeCount > 0 && scopes[scopeCount - 1].elm == null)
			scopeCount--;
		int h = (int) (winSize.height - circuitArea.height * camera.scale);
		pos = 0;
		for (i = 0; i != scopeCount; i++)
			scopeColCount[i] = 0;
		for (i = 0; i != scopeCount; i++) {
		...
		// if (scopeCount>0){
		// knob.setRect(new
		// Rectangle(scopes[scopeCount-1].rightEdge()+10,winSize.height -
		// h,h,h));
		// }else{
		// knob.setRect(new Rectangle(winSize.width-h-10,winSize.height -
		// h,h,h));
		// }
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == resetButton) {
			...
			dbimage = pnlMain.createImage(winSize.width, winSize.height);
		...
		if (e.getSource() == dumpMatrixButton)
			dumpMatrix = true;
		if (e.getSource() == exportItem || e.getSource() == btnSave)
			doExport();
		if (e.getSource() == optionsItem)
			doEdit(new EditOptions(this));
		if (e.getSource() == importItem || e.getSource() == btnOpen)
			doImport();
		if (e.getSource() == exportLinkItem)
			doExportAs(false);
		if (e.getSource() == undoItem || e.getSource() == undoItemPopUp
				|| e.getSource() == btnUnDo)
			doUndo();
		if (e.getSource() == redoItem || e.getSource() == redoItemPopUp
				|| e.getSource() == btnReDo)
			doRedo();
		if (ac.compareTo("Cut") == 0 || e.getSource() == btnCut) {
			if (e.getSource() != elmCutMenuItem)
				menuElm = null;
			doCut();
		}
		if (ac.compareTo("Copy") == 0 || e.getSource() == btnCopy) {
			if (e.getSource() != elmCopyMenuItem)
				menuElm = null;
			doCopy();
		}
		if (ac.compareTo("Paste") == 0 || e.getSource() == btnPaste)
			doPaste();
		...
		if (ac.compareTo("Delete") == 0 || e.getSource() == btnDelete) {
		...
		if (e.getSource() == btnSpeedUp) {
			if (speedBar.getValue() < speedBar.getMaximum() - 2) {
				speedBar.setValue(speedBar.getValue() + 3);
			} else {
				btnZoomIn.setEnabled(false);
			}
			if (!btnZoomOut.isEnabled())
				btnZoomOut.setEnabled(true);
			if (currentBar.getValue() < currentBar.getMaximum())
				currentBar.setValue(currentBar.getValue() + 1);
		}

		if (e.getSource() == btnSpeedDown) {
			if (speedBar.getValue() > speedBar.getMinimum() + 2) {
				speedBar.setValue(speedBar.getValue() - 3);
			} else {
			...
			if (currentBar.getValue() > currentBar.getMinimum())
				currentBar.setValue(currentBar.getValue() - 1);
		}
		...
		if (e.getSource() == btnNew || e.getSource() == newItem) {
			clearCircuit();
			t = 0;
			cv.repaint();
		}
		...
		if (e.getSource() == this.tglMoveBody) {
			setMouseMode(MODE_SELECT);
			mouseModeStr = "Select";
			for (int j = 0; j < 24; j++) {
				this.tglAddElements[j].setSelected(false);
			}
			this.tglMoveBody.setSelected(true);
		}
		...
		// char ch=174;
		// System.out.print("char 174=="+ch);
		if (ac.startsWith("addelm")) {
			...
			case 10:
				...
				break;
			case 11:
				c = shortcuts['o']; // 2way switch switch2elm
				break;
			case 12:
				c = shortcuts['y'];
				break;
			...
			this.tglMoveBody.setSelected(false);
			this.tglAddElements[i].setSelected(true);
	...
	void doImport() {
		...
		pushUndo();
		impDialog.execute();
		this.setTitle("SymCiruit: " + currentFileTitle);
	}
	...
	void doExport() {
		if (currentFilePath != "") {
			try {
				...
				ImportExportFileDialog.writeFile(currentFilePath);
				return;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	void clearCircuit() {
		...
		knob.setElement(null);
		elmList.removeAllElements();
		hintType = -1;
		timeStep = 5e-6;
		dotsCheckItem.setState(true); // hausen: changed from true to false
		smallGridCheckItem.setState(false);
		powerCheckItem.setState(false);
		voltsCheckItem.setState(true);
		showValuesCheckItem.setState(true);
		setGrid();
		speedBar.setValue(150); // 57
	...
	void readSetup(String text, boolean retain) {
		...
		titleLabel.setText("untitled");
		// currentFileTitle=text;
		// this.setTitle("SymCiruit: "+text);
	}
	...
	void readSetup(byte b[], int len, boolean retain) {
		...
		if (!retain) {
			...
			dotsCheckItem.setState(false); // hausen: changed from true to false
			smallGridCheckItem.setState(false);
			drawGridCheckItem.setState(true);
			powerCheckItem.setState(false);
			voltsCheckItem.setState(true);
			showValuesCheckItem.setState(true);
			setGrid();
	...
	public void mouseDragged(MouseEvent e) {
		...
		camera.inverseTransform(pt);

		if (!circuitArea.contains(pt.x, pt.y))
			return;

		if (dragElm != null) {
			dragElm.drag(pt.x, pt.y);
		}

		boolean success = true;
		switch (tempMouseMode) {
		case MODE_DRAG_ALL:
			dragAll(snapGrid(pt.x), snapGrid(pt.y));
			break;
		case MODE_DRAG_ROW:
			dragRow(snapGrid(pt.x), snapGrid(pt.y));
			break;
		case MODE_DRAG_COLUMN:
			dragColumn(snapGrid(pt.x), snapGrid(pt.y));
			break;
		case MODE_DRAG_POST:
			if (mouseElm != null)
				dragPost(snapGrid(pt.x), snapGrid(pt.y));
			break;
		case MODE_SELECT:
			if (mouseElm == null)
				...
				tempMouseMode = MODE_DRAG_SELECTED;
				success = dragSelected(pt.x, pt.y);
			}
			break;
	...
	void selectArea(int x, int y) {
		...
		Rectangle r = new Rectangle((int) (x1 / camera.scale),
				(int) (y1 / camera.scale), (int) ((x2 - x1) / camera.scale),
				(int) ((y2 - y1) / camera.scale));
		int i;
		for (i = 0; i != elmList.size(); i++) {
			CircuitElm ce = getElm(i);
			ce.selectRect(r);
	...
	void removeZeroLengthElements() {
		...
		for (i = elmList.size() - 1; i >= 0; i--) {
			...
			if (ce.x == ce.x2 && ce.y == ce.y2) {
				if (elmList.elementAt(i) == knob.element)
					knob.setElement(null);
	...
	public void mouseMoved(MouseEvent e) {
		if (knob.element != null) {
			if (knob.rect.contains(e.getPoint())) {
				mouseElm = knob.element;
				return;
			}
		}
		...
		for (i = 0; i != elmList.size(); i++) {
			...
			if (ce.boundingBox.contains(x, y)) {
				...
				for (j = 0; j != jn; j++) {
					...
					int dist = distanceSq(x, y, p.x, p.y);
		...
		// set point as untransformed
		pt = e.getPoint();
		scopeSelected = -1;
		if (mouseElm == null) {
			for (i = 0; i != scopeCount; i++) {
				Scope s = scopes[i];
				if (s.rect.contains(pt.x, pt.y)) {
					s.select();
					scopeSelected = i;
				}
			}
			...
			for (i = 0; i != elmList.size(); i++) {
				...
				for (j = 0; j != jn; j++) {
					Point p = ce.getPost(j);
					distanceSq(x, y, p.x, p.y);
					if (distanceSq(p.x, p.y, x, y) < 26) {
						mouseElm = ce;
						mousePost = j;
						break;
					}
	...
	public void mousePressed(MouseEvent e) {
		...
		if ((ex & (MouseEvent.META_DOWN_MASK | MouseEvent.SHIFT_DOWN_MASK)) == 0
			...
			if (e.getY() > circuitArea.height * camera.scale) {
				avoidDrag = true;
				return;
			}
		}
		...
		if ((e.getModifiers() & MouseEvent.BUTTON1_MASK) != 0) {
			...
			if ((ex & MouseEvent.ALT_DOWN_MASK) != 0
				...
				tempMouseMode = MODE_DRAG_ROW; // shifts points on row up or
												// down
		...
		if (mouseMode == MODE_SELECT && mouseElm != null)
			knob.setElement(mouseElm);

		Point pt = new Point(e.getPoint());
		camera.inverseTransform(pt);
		if (mouseMode == MODE_SELECT && doSwitch(pt.x, pt.y)) {
			didSwitch = true;
			return;
		}
		pushUndo();
		pt = new Point(e.getPoint());
		initDragX = pt.x;
		initDragY = pt.y;
		dragging = true;
		camera.inverseTransform(pt);
		if (tempMouseMode != MODE_ADD_ELM || addingClass == null)
			return;
		// pt=new Point(e.getPoint());
		int x0 = snapGrid(pt.x);
		int y0 = snapGrid(pt.y);
	...
	void setZoom(boolean up){
		...
		int h = winSize.height / 5;
		// if (camera.scale <1)
		int x = snapGrid(circuitArea.width / 2);
		int y = snapGrid(circuitArea.height / 2);
		circuitArea = new Rectangle(0, 0, (int) (winSize.width / camera.scale),
				(int) ((winSize.height - h) / camera.scale));
		x -= snapGrid(circuitArea.width / 2);
		y -= snapGrid(circuitArea.height / 2);
		for (int i = 0; i != elmList.size(); i++) {
			CircuitElm ce = getElm(i);
			ce.move(-x, -y);
		}
		analyzeCircuit();
		cv.invalidate();
		statusBar.setStatusLine("Zoom = " + Math.round(camera.getScale() * 100)
				+ "%");
	...
	public void mouseWheelMoved(MouseWheelEvent me) {
		...
		int scroll = me.getWheelRotation();
		setZoom(scroll>0);
	}
	...
	void doPopupMenu(MouseEvent e) {
		...
		if (scopeSelected != -1) {
			...
			menuElm = mouseElm;
			// jMnuItems[0].setEnabled(mouseElm.getEditInfo(0) != null);
			// jMnuItems[1].setEnabled(mouseElm.canViewInScope());
			// elmPopUp.setDefaultLightWeightPopupEnabled(false);
			// elmPopUp.show(e.getComponent(), e.getX(), e.getY());
		} else {
			doMainMenuChecks(mainMenu);
	...
	void setTglBtnState() {
		...
		if (mouseModeStr.compareTo("WireElm") == 0) {
			tglAddElements[0].setSelected(true);
		} else if (mouseModeStr.compareTo("ResistorElm") == 0) {
			tglAddElements[1].setSelected(true);
		} else if (mouseModeStr.compareTo("CapacitorElm") == 0) {
			tglAddElements[2].setSelected(true);
		} else if (mouseModeStr.compareTo("InductorElm") == 0) {
			tglAddElements[3].setSelected(true);
		} else if (mouseModeStr.compareTo("DCVoltageElm") == 0) {
			tglAddElements[4].setSelected(true);
		} else if (mouseModeStr.compareTo("CurrentElm") == 0) {
			tglAddElements[5].setSelected(true);
		} else if (mouseModeStr.compareTo("ACVoltageElm") == 0) {
			tglAddElements[6].setSelected(true);
		} else if (mouseModeStr.compareTo("RailElm") == 0) {
			tglAddElements[7].setSelected(true);
		} else if (mouseModeStr.compareTo("GroundElm") == 0) {
			tglAddElements[8].setSelected(true);
		} else if (mouseModeStr.compareTo("PotElm") == 0) {
			tglAddElements[9].setSelected(true);
		} else if (mouseModeStr.compareTo("SwitchElm") == 0) {
			tglAddElements[10].setSelected(true);
		} else if (mouseModeStr.compareTo("Switch2Elm") == 0) {
			tglAddElements[11].setSelected(true);
		} else if (mouseModeStr.compareTo("AmmeterElm") == 0) {
			tglAddElements[12].setSelected(true);
		} else if (mouseModeStr.compareTo("VoltmeterElm") == 0) {
			tglAddElements[13].setSelected(true);
		} else if (mouseModeStr.compareTo("OutputElm") == 0) {
			tglAddElements[14].setSelected(true);
		} else if (mouseModeStr.compareTo("GalvanometerElm") == 0) {
			tglAddElements[15].setSelected(true);
		} else if (mouseModeStr.compareTo("TransformerElm") == 0) {
			tglAddElements[16].setSelected(true);
		} else if (mouseModeStr.compareTo("BulbElm") == 0) {
			tglAddElements[17].setSelected(true);
		} else if (mouseModeStr.compareTo("DiodeElm") == 0) {
			tglAddElements[23].setSelected(true);
		} else {
			this.tglMoveBody.setSelected(false);
		}
	...
	void enableUndoRedo() {
		...
		redoItemPopUp.setEnabled(redoStack.size() > 0);
		undoItemPopUp.setEnabled(undoStack.size() > 0);
	...
	void doCut() {
		...
		for (i = elmList.size() - 1; i >= 0; i--) {
			...
			if (ce.isSelected()) {
				clipboard += ce.dump() + "\n";
				if (knob.element == elmList.elementAt(i))
					knob.setElement(null);
				ce.delete();
				elmList.removeElementAt(i);
			}
	...
	void enablePaste() {
		...
		pasteItemPopUp.setEnabled(clipboard.length() > 0);
	}
	...
	void doPaste() {
		...
		if (oldbb != null && newbb != null && oldbb.intersects(newbb)) {
			...
			int dx = 0, dy = 0;
			int spacew = (int) (circuitArea.width * camera.scale - oldbb.width - newbb.width);
			int spaceh = (int) (circuitArea.height * camera.scale
					- oldbb.height - newbb.height);
			if (spacew > spaceh)
	...
	private void exportPNG() {
		...
		if (fileName == null)
			return;
		try {
			cv.saveImage(fileName);
		} catch (IOException e) {
			JOptionPane.showMessageDialog(this, "Could not export to file "
	 ...
	 * 
	 * @param save
	 *            flag to select opn/save dialogue
	 * @param ext
	 *            extention of file (ext (like ".ser") is ignored for
	 *            save=false)
	 * @return
	 */
